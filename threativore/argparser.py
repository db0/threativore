...
arg_parser.add_argument('--refresh_comment', action='store', required=False, type=str, help="Try to only refresh a specific governance comment")
arg_parser.add_argument('--refresh_all_gpost_comments', action='store', required=False, type=str, help="Try to only refresh a specific governance comment, provide a comma-separated list of comment IDs. First the comment to reply,then its parent")
